...
public class LoginDao 
{
	...
	public static boolean register(String name,String email,String pass,String dob,String number,String gender)
	{
		...
		try
		{
			...
			sql="insert into userlogin (username,password,email,dob,gender,mobnum)values " +
					"('"+name+"','"+pass+"','"+email+"','"+dob+"','"+gender+"','"+number+"')";
	...
	public static ResultSet getcounts(String keyword)
	{
		ResultSet resultSet=null;
		try
		{
			con=DBConnection.getConnection();
			st=con.createStatement();
			resultSet = st.executeQuery("select positive,neutral,negative from m_expand_keywords where expand_keyword='"+keyword+"'");
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in UserDAO-getProfile()....."+e);
...
public static  ResultSet  getm_tweets(String college) 
	
	{
		...
		try
		{
			...
			st=con.createStatement();
			 ...
			 resultSet = st.executeQuery( "select id,tweettext from m_tweets where college= '"+college+"'");
			/*while(resultSet.next())
			{
				 data.add(resultSet.getString(1));
			}*/
	...
	public static boolean chnagePass(String pass, String name)
	{
		...
		try
		{
			con=DBConnection.getConnection();
			st=con.createStatement();
			int i=st.executeUpdate("update userlogin set password='"+pass+"' where username='"+name+"'");
			if(i!=0)
			{
				flag=true;
			}
	...
	public static boolean storefiltereddata(int id, String college, String s) {
		try
		{
			...
			st=con.createStatement();
			Statement st1=con.createStatement();
			sql="insert into m_tweet_keywords(id,college,tweettext) values('"+id+"','"+college+"','"+s+"')";
			System.out.println("Sql....."+sql);
			st.executeUpdate(sql);
